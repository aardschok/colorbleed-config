...
def _set_cache_file_path(node, path):
    ...
    try:
        ...
        cmds.connectAttr(plug, "{0}.cachePath".format(node), force=True)
    finally:
...
def cache_file_paths(mapping):
    ...
    original = dict()
    for node in mapping:
        original[node] = cmds.getAttr(node + ".cachePath")
...
class ExtractInstancerMayaAscii(pyblish_magenta.api.Extractor):
